...
class HomeModel() {
    ...
    suspend fun getUserLeagues(userId: String): List<League> = withContext(Dispatchers.IO) {
        try {
            supabase
                .from("leagues")
                .select() {
                    filter {
                        ...
                        eq("league_id","user_league.league_id")
    ...
    suspend fun insertLeagueAndReturn(league: HomeViewModel.LeagueCreationDTO): League? {
        return try {
            supabase
                .from("leagues")
                .insert(league) {
