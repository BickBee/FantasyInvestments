...
object TransactionRouter {
    ...
    suspend fun createTransaction(transaction: Transaction): Result<Unit> =
        withContext(Dispatchers.IO) {
            try {
                val userCash = SessionRouter.getSessionBalance(transaction.league_id, transaction.uid)
                val stockQuantity = StockRouter.getStockQuantity(transaction.league_id, transaction.uid, transaction.stock_id)

                val postTransactionBalance: Double = when (transaction.action) {
                    "BUY" -> {
                    ...
                    "SELL" -> {
                        ...
                        return@withContext Result.failure(
                            IllegalArgumentException("Invalid transaction action: ${transaction.action}")
                        )
                    }
                }

                // Calculate new stock quantity based on the transaction action.
                // Note: Remove any accidental extra computation (the extra line in your BUY branch).
                val postTransactionStockQuantity: Double = when (transaction.action) {
                    "BUY" -> {
                ...
                databaseConnector.supabase
                    ...
                    .insert(transaction) {
                        // Exclude txn_id since it's auto-generated.
                        select()
                    }
    ...
    suspend fun getStockTransactionsForUser(uid: String, leagueId: Int, stockId: Int): List<Transaction> {

        val stockTransactions = databaseConnector.supabase.from(TRANSACTION_TABLE_NAME).select {
            filter {
                eq("stock_id", stockId)
                eq("uid", uid)
